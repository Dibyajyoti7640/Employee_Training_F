...
const EmployeeQuiz = () => {
  ...
  const { user } = useSelector((state) => state.auth); // Assuming user is stored in Redux state
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setSubmittedQuizzes(
          new Set(userSubmissions.map((sub) => sub.assessmentId))
        );
      } catch (error) {
            ...
            {["Option A", "Option B", "Option C", "Option D"].map((option) => (
              <label key={option} className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    handleAnswerChange(quiz.assessmentId, e.target.value)
                  }
      ...
      <div className="text-center p-8">
        ...
        <div className="p-6 bg-gray-50 rounded-xl border border-gray-200">
          ...
          <p className="text-gray-600">
            No quizzes available for your registered courses.
          </p>
        </div>
      </div>
    );
          ...
          <div className="mb-6">
            ...
            <span
              className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${
                selectedQuiz.answerType === "Multiple Choice"
                  ? "bg-indigo-100 text-indigo-800"
                  : selectedQuiz.answerType === "Short Answer"
