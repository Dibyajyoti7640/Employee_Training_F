...
const EmployeeProgress = () => {
  ...
  const handleViewDetails = async (programId) => {
    try {
      if (courses[programId] && courses[programId].description) {
        window.location.href = courses[programId].description;
      } else {
        ...
        if (courseData.description) {
          window.location.href = courseData.description;
        } else {
          navigate(`/dashboard/employee/course/${programId}`);
        }
      }
    } catch (err) {
  ...
  const handleUnregister = async (registrationId, programId) => {
    try {
      const confirmUnregister = window.confirm(
        "Are you sure you want to unregister from this course? Any progress will be lost."
      );
                ...
                <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 flex justify-between items-center">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className="px-3 py-1.5 bg-indigo-600 text-sm text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center shadow-sm hover:shadow"
                    >
                      View Details
